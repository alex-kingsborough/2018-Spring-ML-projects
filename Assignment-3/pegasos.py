...
def objective_function(X, y, w, lamb):
	...
	first_term = LA.norm(w)
	first_term = first_term*first_term
	first_term = lamb/2*first_term
	...
	for i in range(y_size):
		a = np.matmul(np.transpose(w), X[i,:])
		loss = 1 - y[i]*a
		for j in loss:
			if(j < 0):
				j = 0
	...
	obj_value = tot/y_size

	obj_value = first_term+obj_value
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
	...
	for iter in range(1, max_iterations + 1):
		...
		tot = np.zeros(w.shape)

		for curr in A_t:
			a = np.matmul(np.transpose(w), Xtrain[curr,:])
			score = ytrain[curr]*a
			if(score < 1):
				b = Xtrain[curr] * ytrain[curr]
				idk = np.transpose(tot) + np.transpose(b)
				tot = np.transpose(idk)

		eta = 1/(lamb*iter)
		tot = tot*eta/k
		w = (1 - eta*lamb)*w 
		bs = np.transpose(w) + np.transpose(tot)
		w = np.transpose(bs)

		test_val = np.sqrt(lamb)
		test_val = 1/test_val

		test_val = test_val/LA.norm(w)
		if(test_val > 1):
			test_val = 1
		w = test_val*w
